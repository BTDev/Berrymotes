...
(function ($) {
        ...
        if (!isSearch && Bem.effects) {
            $.each(emotes, function (index, emoteDom) {
                ...
                for (var i = 0; i < flags.length; ++i) {
                    ...
                    const apngMatch = flags[i].match(Bem.apngSpeedRegex);
                    if (apngMatch) {
                        var matchedSpeed = parseFloat(apngMatch[1]);
                        if (!isNaN(matchedSpeed))
                            postprocess.speed = matchedSpeed;
                    }
                }
        ...
        $.each(emotes, function (index, emoteDom) {
            ...
            if ($emote.is('.canvasapng') == false) {
                ...
                if (Object.keys(postSettings).length) {
                    postProcessEmote(emote, postSettings)
                        .then(
                            function ok(dataUrl) {
                                $emote.css("background-image", dataUrl);
                            },
                            function notOk(e) {
                                console.error(`Could not do post-processing for emote ${emote.names.join(", ")}!`);
                                console.error(e);
                                $emote[0].style.background = `url(${emote['background-image'] || emote['apng_url']})`;
                            });
                } else {
    ...
    async function postProcessEmote(emote, postprocess) {
        ...
        if (emote.hasOwnProperty("apng_url")) {
            if (postprocess.hasOwnProperty("speed")) {
                try {
                    buffer = setApngSpeed(buffer, postprocess.speed);
                } catch (e) {
                    console.error(e);
                }
            }
    ...
    function setApngSpeed(buffer, speed) {
        ...
        parseChunks(view, (type, _, offset, length) => {
            ...
            const newDelay = Math.round((delayNumerator / delayDenomerator) * speed * 1000) / 1000;
            const [newNumerator, newDenominator] = convertDecimalToFraction(newDelay);

            data.setUint16(offset + 8 + 20, newNumerator);
            data.setUint16(offset + 8 + 22, newDenominator);
            ...
            const newCrc = Bem.Crc32(view.slice(offset + 4, offset + 4 + 4 + length));
    ...
    function getGreatedCommongDenom(a, b) {
        if (b < 0.0000001)
            return a;
        return getGreatedCommongDenom(b, Math.floor(a % b));
    }
